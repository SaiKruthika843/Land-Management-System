# Install pip using `npm i pip`
# Install ecdsa using `pip i ecdsa`
...
# resourses for ecdsa and signing key: https://www.educative.io/answers/how-to-create-digital-signature-in-python-using-ecdsa-signingkey
...
class BlockChain(object):
    ...
    def new_block(self, proof, previous_hash=None):
        ...
        if (len(self.pending_transactions) != 0):
            ...
            block['merkel_root'] = self.calculate_hash(self.calculate_hash(
                self.pending_transactions[0]) + self.calculate_hash(self.pending_transactions[1]))
    ...
    def add_Transaction(self, buyerID, sellerID, propertyID, amount):
        ...
        if (int(buyerID) in self.dict_obj) and (int(buyerID) != int(sellerID)) and (int(sellerID) == 0 or (int(sellerID) in self.dict_obj)) and (int(sellerID) == 0 or int(propertyID) in self.property_owner) and (int(sellerID) == 0 or self.property_owner[int(propertyID)] == int(sellerID)):
            ...
            print(
                'Transaction failed: A user cannot be buyer and seller at the same time')
            return
    ...
    def add_User(self, userId, userName, balance):
        ...
        while (check == 1):
            print(
                'Enter the id of the property that you currently own, press "." if you do not own any more properties: ')
            inp = input()
            if (inp == '.'):
                check = 0
                break
...
loop = 1
while (loop == 1):
    print('\nPlease Enter: \n1 For The Registration Of A New User, \n2 To Create A Transaction, \n3 To Check The Transaction History Of A Property (enter the property id), \n4 To Display The Blockchain, \n5 To Verify The Blockchain, \n6 To Exit\n')
    c = int(input())
    if (c == 6):
        loop = 0
    elif (c == 1):
